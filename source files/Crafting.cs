...
namespace NotAwesomeSurvival {
    ...
    public partial class Crafting {
        ...
        static NasBlock[,] GetArea(NasLevel nl, ushort startX, ushort startY, ushort startZ, Station.Orientation ori) {
            ...
            if (WE) {
                for (ushort y = startY; y > startY - 3; y--) {
                    for (ushort x = startX; x < startX + 3; x++) {
                        BlockID blockID = nl.lvl.GetBlock(x, y, startZ);
                ...
                for (ushort y = startY; y > startY - 3; y--) {
                    for (ushort z = startZ; z < startZ + 3; z++) {
                        BlockID blockID = nl.lvl.GetBlock(startX, y, z);
                        if (blockID == Block.Invalid) { blockID = 0; }
                        BlockID num = blockID;
                        if (blockID >= 256) {
							num = Block.ToRaw(blockID);
        ...
        public class Recipe {
            ...
            public static void FillDict(BlockID ID, ref Dictionary<BlockID, int> stacks) {
            ...
            private bool TestRecipe(NasBlock[,] area, int offsetX, int offsetY, bool mirrored) {
